...
/*   Updated: 2024/01/26 15:05:00 by qbeukelm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	make_command_and_children(t_parse *p)
{
	...
	while (p->tokens_c)
	{
		if (p->ast_c == NULL)
		{
			printf("making cmd with %s\n", p->tokens_c->value);
			make_command_node(p);
		}
		if (p->tokens_c->type == ARGUMENT)
		{
			printf("making arg with %s\n", p->tokens_c->value);
			make_parent_node(p);
...
bool	make_parent_node(t_parse *p)
{
	if (p->ast_c->children == NULL)
		p->ast_c->children = malloc(sizeof(t_ast_node) * count_children(p->tokens_c));
	return (SUCCESS);
}
...
t_ast_node	*tokens_to_parser(t_token *tokens_root)
{
	...
	p = init_parse(tokens_root, NULL); //TODO free
